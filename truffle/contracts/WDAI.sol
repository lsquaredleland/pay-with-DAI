...
contract WDAI {
	...
	// The roundabout way would be to call `transferFrom()` to `DelegateBank` then `withdraw()` then `transfer()` back..
	// function withdrawTo(address dst, uint wad) public returns (bool) {
